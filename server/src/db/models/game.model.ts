...
export const save = async (game: Game) => {
    try {
        ...
        // Handle black player (AI or real user)
        if (game.black?.id === -1) {
            // AI player
            black.id = undefined; // Set black_id to null for AI
            black.name = game.black?.name; // Use name for AI
        } else if (typeof game.black?.id === "string") {
        ...
        const res = await db.query(
            `INSERT INTO "game"(winner, end_reason, pgn, white_id, white_name, black_id, black_name, started_at, ended_at) 
            ...
            [
                ...
                white.id || null,
                white.name || null,
                black.id || null, // Null for AI
                black.name || null,
                new Date(game.startedAt as number),
                new Date(game.endedAt as number),
            ]
        );
        ...
        console.error("save: Error saving game to database", err);
        return null;
    }
};
